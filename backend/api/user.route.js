...
router.post('/:id/contacts', async (req, res) => {
  try {
    ...
    const contactUser = await userService.getUserById(contactUserId);
    if (!contactUser) {
      return res.status(404).json({ error: 'User not found' });
    }
    
    const result = await userService.addContact(id, contactUserId);
...
router.get('/:id/contacts', async (req, res) => {
  try {
    ...
    const contacts = await userService.getContacts(id);
...
router.get('/:id/contacts/:contactId/check', async (req, res) => {
  try {
    const { id, contactId } = req.params;
    
    const isContact = await userService.isContact(id, contactId);
